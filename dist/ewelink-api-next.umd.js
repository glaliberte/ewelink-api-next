(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('axios'), require('crypto'), require('dayjs'), require('ws'), require('bonjour-service'), require('crypto-js'), require('log4js'), require('path')) :
  typeof define === 'function' && define.amd ? define(['axios', 'crypto', 'dayjs', 'ws', 'bonjour-service', 'crypto-js', 'log4js', 'path'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global["ewelink-api-next"] = factory(global.axios, global.crypto, global.dayjs, global.WebSocket, global.bonjourService, global.CryptoJS, global.log4js, global.path));
})(this, (function (axios, crypto, dayjs, WebSocket, bonjourService, CryptoJS, log4js, path) { 'use strict';
  ...
  const nonce = (size = 8) => Math.random().toString(36).slice(-size);
  /**
   * Hmac-Sha256 Sign
   *
   ...
   */
  const sign = (msg, appSecret, isFormat = false) => {
  ...
  const creatRequest = (config, logObj) => {
      // 创建axios的实例对象，添加全局配置
      // Create an instance object of Axios and add a global configuration
      const instance = axios__default["default"].create(config || {
          baseURL: "",
          timeout: 20000
      });
      // 请求拦截
      ...
      instance.interceptors.request.use((req) => {
          ...
          if (req.headers) {
              ...
              req.headers["Date"] = String(dayjs__default["default"]().valueOf());
          }
          if (logObj) {
      ...
      instance.interceptors.response.use((res) => {
          ...
          const responseTime = res.config?.headers?.Date
              ? parseInt(String(dayjs__default["default"]().valueOf())) - parseInt(res.config.headers?.Date)
              : null;
  ...
  class CreateLoginUrl {
      ...
      createLoginUrl(options) {
          const seq = dayjs__default["default"]().valueOf();
          const params = {
  ...
  class WebAPI extends eWeLinkBase {
      constructor() {
          super(...arguments);
          // 账号管理接口
  ...
  class Connect {
      constructor(root) {
          ...
          this.create = async (options, onOpen, onClose, onError, onMessage) => {
              ...
              if (options?.fullUrl) {
                  ws = new WebSocket__default["default"](options.fullUrl);
              }
              else {
                  if (!options?.region)
                      throw new Error("region is required");
                  ws = new WebSocket__default["default"](await this.getDispatch({ region: options.region }));
              }
              // Set onopen event
              ws.onopen = () => {
  ...
  class Lan {
      ...
      discovery(onDiscover, type = "ewelink") {
          const bonjourClient = new bonjourService.Bonjour();
      ...
      encrypt(data, secretKey, iv) {
          if (!data)
              return "";
          const cipher = CryptoJS__default["default"].AES.encrypt(JSON.stringify(data), CryptoJS__default["default"].enc.Hex.parse(secretKey), {
              iv: CryptoJS__default["default"].enc.Base64.parse(iv),
              mode: CryptoJS__default["default"].mode.CBC,
              padding: CryptoJS__default["default"].pad.Pkcs7
          });
          return cipher.ciphertext.toString(CryptoJS__default["default"].enc.Base64);
      }
      /**
       * data decrypt
      ...
      decrypt(data, secretKey, iv) {
          if (!data)
              return {};
          return JSON.parse(CryptoJS__default["default"].AES.decrypt(data, CryptoJS__default["default"].enc.Hex.parse(secretKey), {
              iv: CryptoJS__default["default"].enc.Base64.parse(iv),
              mode: CryptoJS__default["default"].mode.CBC,
              padding: CryptoJS__default["default"].pad.Pkcs7
  ...
  const createLogger = (name, logLevel, filename) => {
      log4js__default["default"].configure({
          appenders: {
              ...
              httpLog: {
                  type: "dateFile",
                  filename: filename || path__default["default"].join("logs", "default.log"),
                  pattern: "yyyy-MM-dd",
                  keepFileExt: true,
      ...
      return log4js__default["default"].getLogger(name);
  };

  var index = { WebAPI, Ws, Lan, createLogger, creatRequest };
